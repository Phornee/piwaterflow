...
from baseutils_phornee import Logger, Logger_Mode
...
class Waterflow():
    ...
    def __init__(self, template_config_path: str = None):
        self.homevar = os.path.join(str(Path.home()), "var", self.getClassName())
        self.userlogger = Logger(package_name= self.getClassName(), 
                            log_file_name='waterflow', 
                            mode=Logger_Mode.FILE)
        
        self.debuglogger = Logger(package_name= self.getClassName(), 
                                log_file_name='loop', 
                                mode=Logger_Mode.BOTH)

        if not template_config_path:
            template_config_path = os.path.join(Path(__file__).parent, 'config-template.yml')

        self.config = WaterflowConfig(package_name=self.getClassName(), 
        ...
        try:
            ...
            self.userlogger.warning('Error getting connection credentials to InfluxDB.')
            pass
        ...
        try:
            ...
            self.userlogger.warning('Connection to InfluxDB failed: %s.' % host)
            self.conn = None
            pass
    ...
    def _getHomePath(self, subpath: str):
        return os.path.join(self.homevar, subpath)
    ...
    def _readLastProgramTime(self):
        last_program_path = self._getHomePath('lastprogram.yml')
        ...
        try:
            ...
            time_utc_str = self._timeToStr(last_program_time)
            with open(last_program_path, 'w') as file:
                file.writelines([time_utc_str, time_utc_str])
        return last_program_time

    def _writeLastProgramTime(self, time_utc_last:str):
        last_program_path = self._getHomePath('lastprogram.yml')
        with open(last_program_path, 'w') as file:
    ...
    def getLastLoopTime(self):
        tokenpath = self._getHomePath('token')
    ...
    def force(self, type_force, value):
        ...
        if (type_force == 'program' and 0 <= value < len(config['programs'])) or \
           (type_force == 'valve' and 0 <= value < len(config['valves'])):
            force_file_path = self._getHomePath('force')
    ...
    def stop(self):
        stop_req_path = self._getHomePath('stop')
    ...
    def _executeValve(self, valve):
        ...
        self.userlogger.info('Valve %s ON.' % valve)

        self._sleep(self.config['max_valve_time']*60)

        GPIO.output(valve_pin, GPIO.LOW)
        self.userlogger.info('Valve %s OFF.' % valve)
        # if inverter_enable: # If we dont have external 220V power input, then activate inverter
    ...
    def _executeProgram(self, program_number):
        ...
        """
        self.userlogger.info('Executing program %s.' % program_number)
        # inverter_enable =  not GPIO.input(self.config['external_ac_signal_pin'])
        ...
        for idx, valve_time in enumerate(self.config['programs'][program_number]['valves_times']):
            if valve_time > 0 and not self.stopRequested():
                ...
                GPIO.output(valve_pin, GPIO.HIGH)
                self.userlogger.info('Valve %s ON.' % idx)

                self._sleep(valve_time * 60)

                GPIO.output(valve_pin, GPIO.LOW)
                self.userlogger.info('Valve %s OFF.' % idx)
            else:
                self.userlogger.info('Valve %s Skipped.' % idx)
        # if inverter_enable: # If we dont have external 220V power input, then activate inverter
        GPIO.output(self.config['inverter_relay_pin'], GPIO.LOW)  # INVERTER always OFF after operations
        self.userlogger.info('Inverter relay OFF.')
        ...
        if self.getLock():  # To ensure a single execution despite of cron overlapping
            try:
                ...
                if not self.stopRequested():
                    ...
                    if forced_info:
                        ...
                        if forced_type == "program":
                            self.userlogger.info('Forced program {} executing now.'.format(forced_value))
                            # ------------------------
                ...
                if forced_info:
                    ...
                    os.remove(self._getHomePath('force'))
                ...
                self.userlogger.error("Exception looping: {}".format(str(e)))
                raise
            finally:
                GPIO.cleanup()
                self.releaseLock()
